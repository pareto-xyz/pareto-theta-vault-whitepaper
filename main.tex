...
The product invariant has a particular payoff function that suffers from divergence loss \cite{angeris2019analysis}. In other words, if the price does not deviate too much from the initial price that a LP deposited tokens at, the LP makes a profit. However, if the price changes, the LP loses money as a function of the magnitude of the price change. For LPs with a directional belief in the token price, such a payoff is not optimal.

This begs the question: given a payoff function, what is corresponding invariant such that being an LP in that market would provide said payoff? Above, we showed how to derive a payoff $V$ from an invariant $\psi$
