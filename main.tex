...
% CHART - insert AMM trading to LP function from product deck? Maybe add x*y = k under the chart
...
% CHART - insert RMM LP payoff function (V) 
...
% CHART - shall we add the diagram here?
...
There are two techniques to do so. First, one can estimate volatility from historical data. The challenges of this approach are twofold: past volatility not capturing expectations of future events, and, sensitivity to hyperparameter choices such as the sampling period and frequency, the price information (e.g. close-to-close vs. high-low), and the definition of volatility (e.g. Parkinson vs Rogers-Satchell). 
It is difficult to determine what is ``best'' because different choices affect the estimate significantly. 

% POINT - isn't Deribit DVOL a prediction of future volatility because the options expire in the future?
...
% shall we use the same delta notion as earlier in the paper?
