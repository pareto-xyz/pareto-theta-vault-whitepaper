...
As we can see in \label{fig:ammpayoff} below, the product invariant has a payoff function that suffers from divergence loss \cite{angeris2019analysis}. If the price does not deviate too much from the initial price that a LP deposited tokens at, the LP makes a profit vs. just holding the underlying asset. However, if the price changes a lot, the LP loses money that scales with the magnitude of the price change. For LPs with a directional belief in the token price, such a payoff is not optimal.

\begin{figure}[h!]
    \caption{Automated Market Makers: Constant Product Invariant (Trading Function) and Liquidity Provider Payoff Function}
    \centering
    \includegraphics[width=0.5\linewidth]{ammpayoff.png}
    \caption{The CPMM Trading Function is $xy = k$, where x = reserves of Token X and y = reserves of Token Y.
            The Liquidity Provider payoff function is $V(S) = 2\sqrt{Sk}$ where $S = spot price$ and $k = invariant$.}
    \label{fig:ammpayoff}
\end{figure}
...
The method goes further to show that the payoff function and the invariant are Fenchel conjugates of one another, a surprising result. So, to enable LPs to achieve a payoff $V$, one can design a CFMM with the invariant $\psi_V$. We say that $\psi_V$ ``replicates'' the payoff $V$. The first RMM implementation, a liquidity provider payoff function that replicates the covered call options selling strategy is used to derive a corresponding trading function, as shown in \label{fig:rmmpayoff}. 

\begin{figure}[h!]
    \caption{Example Replicating Market Makers: RMM-01 Invariant (Trading Function) and Liquidity Provider Payoff Function (Covered Call Option)}
    \centering
    \includegraphics[width=0.5\linewidth]{rmmpayoff.png}
    \caption{The RMM-01 Trading Function is $y - K\Phi(\Phi^{-1}(1-x) - \sigma\sqrt{\tau} = k$, where $x = reserves of Token X, y = reserves of Token Y, k = invariant, K = strike price, \sigma = implied volatility, \tau = time to maturity, \Phi = Gaussian CDF$.
    The RMM-01 Liquidity Provider payoff function is $V(S) = S(1-\Phi(d_1)+K\Phi(d_2))$ where $S = spot price, K = strike, \Phi = Gaussian CDF, d_1 = \frac{\log(S/K) + (\sigma^2/2)\tau}{\sigma\sqrt{\tau}}, d_2 = d_1 - \sigma\sqrt{\tau}$.}
    \label{fig:rmmpayoff}
\end{figure}

In Section~\ref{sec:coveredcall}, we focus on this RMM that replicates the payoff of a covered call option selling strategy.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{userexperience.png}
...
\begin{enumerate}
    \item \textbf{User Deposits} - new users seeking yield will deposit risky assets (e.g. ETH) into the Pareto Vault.       
